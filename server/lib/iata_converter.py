...
class IATAConverter:
    ...
    def get_airline_by_IATA(self, iata_code):
        ...
        response = requests.request("GET", BASE_URL + AIRLINE_PATH + iata_code, headers=HEADERS)
    ...
    def get_IATA_by_airline(self, airline):
        ...
        # if not, we need to query the API
        response = requests.request("GET", BASE_URL + AIRLINE_PATH + "search", headers=HEADERS, params={"name": airline})
        response = response.json()
    ...
    def get_airport_by_IATA(self, iata_code): 
        ...
        # if not, we need to query the API
        response = requests.request("GET", BASE_URL + AIRPORT_PATH + iata_code, headers=HEADERS)
        response = response.json()
